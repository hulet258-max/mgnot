...
import ResponsiveImage, { blurredImageStyle } from "./ResponsiveImage";
  ...
  return (
    <article className="raffle-card">
      <button className="raffle-card-open" type="button" onClick={handleOpen}>
        <div className="raffle-card-image item-image-backdrop" style={blurredImageStyle(raffle.coverImageUrl)}><ResponsiveImage path={raffle.coverImageUrl} alt={raffle.itemName} sizes="(max-width: 720px) 50vw, 25vw" /><span className={`raffle-card-draw-status ${isOnDraw ? "drawing" : ""}`}>{isOnDraw ? t("raffle.ui.onDraw", "On draw") : t("raffle.ui.open", "Open")}</span></div>
...
function MainPage() {
  ...
  const resetModal = () => {
    ...
    setPurchase(null); setNumbers([]); setSelectedNumber(null); setModalError(""); setGalleryIndex(0);
  };
  const closeModal = () => {
    if (busy) return;
  ...
  return (
    <TournamentShell>
      ...
      {activeRaffle && createPortal(<div className="modal-backdrop raffle-modal-layer" role="dialog" aria-modal="true" aria-labelledby="raffle-modal-title" aria-busy={busy} onClick={closeModal}><div className={`raffle-modal ${busy ? "is-processing" : ""}`} onClick={(event) => event.stopPropagation()}>
        ...
        {step === "select_number" && selectedNumber && <div className="raffle-selection-pay"><button className="wc-button" type="button" onClick={() => setStep("payment")}>{t("raffle.ui.pay", "Pay")}</button></div>}
